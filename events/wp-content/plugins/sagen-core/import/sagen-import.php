...
class SagenCoreImport {
	...
	function __construct() {
		...
		$this->importURI       = defined( 'SAGEN_SELECT_PROFILE_SLUG' ) ? 'http://export.' . SAGEN_SELECT_PROFILE_SLUG . '-themes.com/' : '';
	...
	public function import_content( $file ) {
		ob_start();
		require_once( SAGEN_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$sagen_import = new WP_Import();
		set_time_limit( 0 );
		
		$sagen_import->fetch_attachments = $this->attachments;
		$returned_value                  = $sagen_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'sagen-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'sagen-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $sagen_widget_id => $sagen_widget_data ) {
			update_option( 'widget_' . $sagen_widget_id, $sagen_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'sagen-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$sagen_sidebars = get_option( "sidebars_widgets" );
		unset( $sagen_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$sagen_sidebars = array_merge( (array) $sagen_sidebars, (array) $data['sidebars'] );
			unset( $sagen_sidebars['wp_inactive_widgets'] );
			$sagen_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $sagen_sidebars );
			$sagen_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $sagen_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'sagen-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_sagen', $options );
		$this->message = esc_html__( 'Options imported successfully', 'sagen-core' );
	}
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $sagen_page_option => $sagen_page_id ) {
			update_option( $sagen_page_option, $sagen_page_id );
		}
	}
	
	public function rev_sliders() {
		$rev_sldiers = array(
			'apartment-complex.zip',
			'left-menu-home.zip',
			'main-home.zip',
            'properties-home.zip',
            'properties-home-building.zip',
            'properties-slider-01.zip',
            'property.zip',
            'property-agency-rev-1.zip',
            'property-agency-rev-2.zip',
            'property-agency-rev-3.zip',
            'property-single-02.zip'
		);
	...
	function qodef_admin_import() {
		if ( sagen_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				esc_html__( 'Sagen Options - Sagen Import', 'sagen-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'sagen-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                SAGEN_SELECT_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $sagen_select_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'sagen_select_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'sagen_select_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'qodef_options_sagen' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );
			
			update_option( 'qodef_options_sagen', $new_global_values );
		}
	}
	
	function qodef_import_get_demo_urls( $folder ) {
		$demo_urls  = array();
		$domain_url = defined( 'SAGEN_SELECT_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . SAGEN_SELECT_PROFILE_SLUG . '-themes.com/' : '';
