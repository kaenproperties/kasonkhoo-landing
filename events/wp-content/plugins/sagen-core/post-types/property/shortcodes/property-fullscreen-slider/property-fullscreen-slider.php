...
class PropertyFullScreenSlider implements Lib\ShortcodeInterface {
	private $base;
	
	public function __construct() {
		$this->base = 'qodef_property_fullscreen_slider';
		
		add_action( 'vc_before_init', array( $this, 'vcMap' ) );
		
		//Property category filter
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_category_callback', array( &$this, 'propertyCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Property category render
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_category_render', array( &$this, 'propertyCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Property selected projects filter
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_selected_projects_callback', array( &$this, 'propertyIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Property selected projects render
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_selected_projects_render', array( &$this, 'propertyIdAutocompleteRender', ), 10, 1 ); // Render exact property. Must return an array (label,value)

		//Property tag filter
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_tag_callback', array( &$this, 'propertyTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Property tag render
		add_filter( 'vc_autocomplete_qodef_property_fullscreen_slider_tag_render', array( &$this, 'propertyTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'     => esc_html__( 'Sagen Property Fullscreen Slider', 'qodef-core' ),
					'base'     => $this->base,
					'category' => esc_html__( 'by SAGEN', 'qodef-core' ),
					'icon'     => 'icon-wpb-property-fullscreen-slider extended-custom-icon',
					'params'   => array(
						array(
							'type'        => 'textfield',
						...
						array(
                            ...
                            'value'       => array(
                                esc_html__( 'Light', 'qodef-core') => 'qodef-pfs-light-skin',
                                esc_html__( 'Dark', 'qodef-core') => 'qodef-pfs-dark-skin'
                            )
                        ),
						array(
							'type'        => 'autocomplete',
	...
	public function render( $atts, $content = null ) {
		$args   = array(
            ...
            'property_fullscreen_slider' => 'yes',
        );
		$params = shortcode_atts( $args, $atts );

		$params['type'] = 'gallery';
		$params['item_style'] = 'fullscreen-slider';
        $params['space_between_items']='no';
        $params['image_proportions']='full';

        $customNavigation  = '<div class="qodef-pfs-custom-nav">';
        $customNavigation .= '<span class="qodef-pfs-custom-nav-prev">' . esc_html__('Previous Project','qodef-core') . '</span>';
        $customNavigation .= '<span class="qodef-pfs-custom-nav-next">' . esc_html__('Next Project','qodef-core') . '</span>';
        $customNavigation .= '</div>';

		$html = '<div class="qodef-property-fullscreen-slider-holder '. $params['skin'] .' ">';
