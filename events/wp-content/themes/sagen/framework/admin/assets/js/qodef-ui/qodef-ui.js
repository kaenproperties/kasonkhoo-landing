(function($){
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'sagen_select_gallery_upload_get_images',
						ids: ids,
	...
	function qodefInitAjaxForm() {
		...
		$(document).delegate(".qodef_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "sagen_select_save_options"
				};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'sagen_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'sagen_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'sagen_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
	...
	function qodefInitImportContent() {
		...
		if (qodefImportHolder.length) {
			...
			var qodefInitContentImport = function(import_expl, progress_bar, progress_value, import_only_content) {
				var xml_file_name = index < 10 ? 'sagen_content_0' + index + '.xml' : 'sagen_content_' + index + '.xml',
				...
				jQuery.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'sagen_core_action_import_content',
						xml: xml_file_name,
						example: import_expl,
						...
						if (index < 10) {
							...
							if (import_only_content === true) {
								...
								jQuery.ajax({
									...
									url: ajaxurl,
									data: {
										action: 'sagen_core_action_import_other_elements',
										example: import_expl
									},
									success: function (data, textStatus, XMLHttpRequest) {
										qodefSetCompletedMessage(progress_value, progress_bar);
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'sagen_core_action_import_widgets',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								qodefSetCompletedMessage(progress_value, progress_bar);
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'sagen_core_action_import_options',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								qodefSetCompletedMessage(progress_value, progress_bar);
