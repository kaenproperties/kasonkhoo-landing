...
include SAGEN_SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( SAGEN_SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( sagen_select_is_customizer_item_enabled( $module_load, 'sagen_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'sagen_select_get_icon_pack_collections' ) ) {
	function sagen_select_get_icon_pack_collections() {
		$options = apply_filters( 'sagen_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'sagen_select_get_icon_pack_options' ) ) {
	function sagen_select_get_icon_pack_options() {
		$options = apply_filters( 'sagen_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'sagen_select_get_icon_pack_social_options' ) ) {
	function sagen_select_get_icon_pack_social_options() {
		$options = apply_filters( 'sagen_select_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: SagenSelectClassIconCollections
 */
class SagenSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = sagen_select_get_icon_pack_collections();
		$this->iconPackOptions       = sagen_select_get_icon_pack_options();
		$this->iconPackSocialOptions = sagen_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = sagen_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'sagen' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'sagen' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'sagen' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'sagen' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'sagen' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'sagen' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'sagen' )       => 'fa-lg',
			esc_html__( 'Small', 'sagen' )      => 'fa-2x',
			esc_html__( 'Medium', 'sagen' )     => 'fa-3x',
			esc_html__( 'Large', 'sagen' )      => 'fa-4x',
			esc_html__( 'Very Large', 'sagen' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iSagenSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'sagen' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'sagen' ) ] = '';
