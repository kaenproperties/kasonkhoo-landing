...
if ( ! function_exists( 'sagen_select_add_font_awesome_icon_pack' ) ) {
	function sagen_select_add_font_awesome_icon_pack( $iconCollections ) {
		$iconCollections['font_awesome'] = new SagenSelectClassIconsFontAwesome( 'Font Awesome', 'fa_icon' );
		
		return $iconCollections;
	}
	
	add_filter( 'sagen_select_filter_add_icon_pack_into_collection', 'sagen_select_add_font_awesome_icon_pack' );
}

if ( ! function_exists( 'sagen_select_add_font_awesome_icon_pack_option' ) ) {
	function sagen_select_add_font_awesome_icon_pack_option( $options ) {
		$options['font_awesome'] = esc_html__( 'Font Awesome', 'sagen' );
		
		return $options;
	}
	
	add_filter( 'sagen_select_filter_add_icon_pack_into_options', 'sagen_select_add_font_awesome_icon_pack_option' );
	add_filter( 'sagen_select_filter_add_icon_pack_into_social_options', 'sagen_select_add_font_awesome_icon_pack_option' );
}

class SagenSelectClassIconsFontAwesome implements iSagenSelectInterfaceIconCollection {
	public $icons;
	public $title;
	public $param;
	public $styleUrl;
	...
	function __construct($title = "", $param = "") {
		...
		$this->styleUrl = SAGEN_SELECT_FRAMEWORK_ICONS_ROOT . '/font-awesome/css/fontawesome-all.min.css';
	}
	...
	private function getNewIconsArray( $brandsIcons = false ) {
		...
		$iconsArray = array();
		$icons      = json_decode( $wp_filesystem->get_contents( SAGEN_SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/font-awesome/icons.json' ) );
