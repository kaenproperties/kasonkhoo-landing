...
/*
   Class: SagenSelectClassMultipleImages
   A class that initializes Select Multiple Images
*/

class SagenSelectClassMultipleImages implements iSagenSelectInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$sagen_select_global_Framework->qodeMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="qodef-gallery-uploader">
								<a class="qodef-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'sagen' ); ?></a>
								<a class="qodef-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'sagen' ); ?></a>
							</div>
...
class SagenSelectClassTwitterFramework implements iSagenSelectInterfaceRender {
	public function render( $factory ) {
		$twitterApi = SagenTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'sagen' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'sagen' ) : esc_html__( 'Connect with Twitter', 'sagen' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="qodef-page-form-section" id="qodef_enable_social_share">
			<div class="qodef-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'sagen' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'sagen' ); ?></p>
			</div>
			<div class="qodef-section-content">
				<div class="container-fluid">
					<div class="row">
...
class SagenSelectClassInstagramFramework implements iSagenSelectInterfaceRender {
    public function render( $factory ) {
        $instagram_api = SagenInstagramApi::getInstance();
        $message       = '';
        ...
        if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'sagen' );
        }
        ...
        if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'sagen' );
        }
        ...
        if ( ! empty( $_GET['disconnect'] ) ) {
            ...
            $message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'sagen' );
        ...
        <div class="qodef-page-form-section" id="qodef_enable_social_share">
            <div class="qodef-field-desc">
                <h4><?php esc_html_e( 'Connect with Instagram', 'sagen' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'sagen' ); ?></p>
            </div>
            <div class="qodef-section-content">
                <div class="container-fluid">
                    <?php
                    ...
                    if ( $instagram_user_id ) { ?>
                        <div class="row">
                            <div class="col-lg-12">
                                <p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'sagen' );
                    ...
                    <div class="row">
                        <?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'sagen' ); ?></a>
                            </div>
                        <?php } else if ( empty( $connection_type ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'sagen' ); ?></a>
                            </div>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'sagen' ); ?></a>
                            </div>
                        <?php } else { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'sagen' ) ?></a>
                            </div>
                        <?php } ?>
                    </div>
                </div>
...
class SagenSelectClassRepeater implements iSagenSelectInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'sagen' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="qodef-repeater-table-heading">
					<div class="qodef-repeater-fields-holder">
						<div class="qodef-repeater-table-cell qodef-repeater-sort"><?php esc_html_e( 'Order', 'sagen' ) ?></div>
						...
						<div class="qodef-repeater-table-cell qodef-repeater-remove"><?php esc_html_e( 'Remove', 'sagen' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="qodef-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="qodef-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = sagen_select_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class SagenSelectClassFieldAddress extends SagenSelectClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Sagen Options', 'sagen' ); ?></p>
							<?php } ?>
							<a class="qodef-reset-marker qodef-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'sagen' ); ?></a>
...
class SagenSelectClassFieldIcon extends SagenSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = sagen_select_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = sagen_select_icon_collections()->getIconCollectionsKeys();
		?>
			...
			<div class="qodef-section-content">
				<div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = sagen_select_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						...
						<div class="row qodef-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
							<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control qodef-form-element">
									<?php
									$icons       = sagen_select_icon_collections()->getIconCollection( $icons_collection );
