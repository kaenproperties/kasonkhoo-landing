...
(function($) {
    ...
    */
    function qodefOnWindowLoad() {
		qodefInitParallax();
		qodefImageMapGalleryPagination();
    ...
    */
	 ...
	 */
	function qodefScrollToContent() {
		var scrollLandingRow = $(".qodef-scroll-landing-row");
		if (scrollLandingRow.length) {
			var scrollLandingRowHeight = scrollLandingRow.height(),
				scrollLandingRowOffset = scrollLandingRow.offset().top,
				scrollLandingRowArea = scrollLandingRowHeight - scrollLandingRowOffset,
				scrollDownButton = scrollLandingRow.find('.svg-scroll-down'),
				pageJump = false,
				normalScroll = true,
				set = false;

			var qodefScrollHandler = function () {
				if ($(window).scrollTop() < scrollLandingRowArea) {
					normalScroll = false;
				}

				function qodefScrollTo() {
					if (!qodef.body.hasClass('qodef-fp-opened')) {
						clearTimeout(buttonTimeout);
						pageJump = true;
						$('html, body').animate({
							scrollTop: scrollLandingRowArea
				...
				var buttonTimeout = setTimeout(function() {
					if (qodef.windowWidth > 1024 && qodef.scroll < scrollLandingRowArea) {
						qodefScrollTo();
					}
				}, 6000);
				...
				window.addEventListener('wheel', function (event) {
					...
					if ($(window).scrollTop() - scrollLandingRowOffset <= Math.round(scrollLandingRowHeight * 0.5)) {
						reInitOneScroll = true;
					}

					if (!pageJump && !normalScroll) {
						if (scrollingForward && ($(window).scrollTop() < scrollLandingRowArea)) {
				...
				$(document).on('mousedown', function (event) {
					if ($(window).outerWidth() <= event.pageX) {
						if ($(window).scrollTop() == scrollLandingRowOffset) {
							event.preventDefault();
							qodefScrollTo();
						}
					}
			...
			window.addEventListener('scroll', function () {
				if (!set) {
					$(window).scrollTop(scrollLandingRowOffset);
				}
			})
	...
	function qodefIwtAppear() {
		var imageWithText = $('.qodef-image-with-text-holder:not(.qodef-has-shadow), .qodef-property-info-item .qodef-pi-image');
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'sagen_select_search_post_types',
                                term: searchTerm,
                                postType: postType,
	...
	function qodefImageMapGalleryPagination() {
		var imageMapHolder = $('.qodef-image-map-gallery');
		
		if (imageMapHolder.length) {
			imageMapHolder.each(function() {
				var thisImageMapHolder = $(this),
					thisImgSlider = thisImageMapHolder.find('.qodef-img-slider'),
					thisImgPaginationSlider = thisImageMapHolder.find('.qodef-pagination-slider'),
					currentItemIndex = 0;

				var setActivePagItem = function(itemIndex) {
					thisImgPaginationSlider.find('.owl-item').removeClass('qodef-active-pag-item');
					thisImgPaginationSlider.find('.owl-item').eq(itemIndex).addClass('qodef-active-pag-item');
				}

				thisImgSlider.on('initialized.owl.carousel translate.owl.carousel', function() {
					setTimeout(function() {
						currentItemIndex = thisImgSlider.find('.owl-item.active').index();
						setActivePagItem(currentItemIndex);
					}, 50);
				})
			});
		}
	...
	function qodefInitBlogPagination(){
		...
		var initMainPagFunctionality = function(thisHolder) {
			...
			if(nextPage <= maxNumPages){
				...
				var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'sagen_select_blog_load_more');
			...
			if(nextPage === maxNumPages){
				thisHolder.find('.qodef-blog-pag-load-more').queue(function (next) {
					$(this).hide();
					next();
				});
			}
...
(function($) {
    ...
    */
    function qodefOnDocumentReady() {
		...
		qodefWrapperAroundWidgetImages();
    }
    ...
    */
	...
	function qodefWrapperAroundWidgetImages() {

		var imgSelector = $('.widget.woocommerce.widget_products ul li a img, .widget.woocommerce.widget_recent_reviews ul li a img, .widget.woocommerce.widget_recently_viewed_products ul li a img, .widget.woocommerce.widget_top_rated_products ul li a img');

			imgSelector.wrap("<div class='qodef-around-image'></div>");
	}
    ...
    function qodefInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'sagen_select_blog_shortcode_load_more');
        ...
        if (popupMenuOpener.length) {
            ...
            popupMenuOpener.on('click',function(e){
                ...
                if (!popupMenuOpener.hasClass('qodef-fm-opened')) {
                    ...
                    $('html').addClass('qodef-fullscreen-menu-opened-html');
                    qodef.body.removeClass(cssClass);
                    qodef.modules.common.qodefDisableScroll();
                    ...
                    $(document).keyup(function(e){
                        if (e.keyCode === 27 ) {
							...
							$('html').removeClass('qodef-fullscreen-menu-opened-html');
                            qodef.body.addClass(cssClass);
                            qodef.modules.common.qodefEnableScroll();
